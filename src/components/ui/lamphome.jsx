...
export function Lamphome({
  ...
  const [glowPosition, setGlowPosition] = useState('below');
  ...
  const calculateGlowPosition = (currentDragY) => {
  ...
  return (
    <div
      ...
      <motion.div
        ...
        <nav
          className="hidden [@media(min-width:640px)]:flex items-center space-x-4 [@media(min-width:768px)]:space-x-6">
          {navItems.map((item, index) => (
            <Link
              ...
              {item.label}
              <span
                className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-blue-500 to-purple-500 group-hover:w-full transition-all duration-300"></span>
            </Link>
          ))}
        </nav>
        ...
        <div
          className="absolute right-3 top-full mt-2 flex flex-col items-center group z-10">
          <motion.div
          ...
          <motion.div
            ...
            <div
              className="w-full h-full rounded-full bg-gradient-to-br from-yellow-300 to-transparent opacity-60"></div>
